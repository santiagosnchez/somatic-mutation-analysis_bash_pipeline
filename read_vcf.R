...
read_vcf <- function (vcfs, samples = NULL, genome_build = c("hg19", "hg38", "mm10", "mm9"), keep_only_pass = FALSE, verbose = TRUE){
  ...
  # maf header
  func_head = scan(gzfile(vcf_with_filter[1]), what=character(), sep="\n", quiet=T)
  ...
  vcfs <- purrr::map(vcfs, ~tryCatch(data.table::fread(., select = c(1, 2, 4, 5, 7, 8), skip = "#CHROM"),
          error = function(e) {
          message("It seems ", ., " has no normal VCF header, try parsing without header.")
          data.table::fread(., select = c(1, 2, 4, 5, 7, 8)) %>% mutate()
          }))
